...
  };
  ...
  const UIManager = {
    ...
    _createReviewHTML(r,i){
      ...
      return `<div class="p-4 border border-gray-200 rounded-lg bg-white flex flex-col" data-index="${i}">
        ${image?`<img src="${image}" class="mb-3 max-h-72 w-full object-cover rounded-md">`:''}
        <div class="flex-grow">
  ...
  const AppController = {
    ...
    setState(s){
      ...
      if(s.reviews!==undefined) UIManager.renderReviews(this.state.reviews);
    },

    async withLoading(fn){
      if(this.state.isBusy) return;
      this.setState({isBusy:true});
      try{await fn();}catch(err){console.error(err);UIManager.showToast(err.message,true);}finally{this.setState({isBusy:false});}
    },
